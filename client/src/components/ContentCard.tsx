...
export default function ContentCard({ item, onDelete }: Props) {
  ...
  return (
    <Link
      to={`/entry/${item._id}`}
      className="block max-w-175 w-[90%] mx-auto my-4"
    >
      <article className="flex items-stretch gap-4 p-4 bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow cursor-pointer">
