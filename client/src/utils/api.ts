...
export const createEntry = async (
  formData: TravelEntryFormData
): Promise<TravelEntry> => {
  const response = await api.post<ApiResponse<TravelEntry>>(
    "/entries",
    formData
  );
  if (!response.data.data) {
...
export const updateEntry = async (
  id: string,
  formData: Partial<TravelEntryFormData>
): Promise<TravelEntry> => {
  const response = await api.put<ApiResponse<TravelEntry>>(
    `/entries/${id}`,
    formData
  );
  if (!response.data.data) {
...
export const deleteEntry = async (id: string): Promise<void> => {
  const response = await api.delete<ApiResponse<{}>>(`/entries/${id}`);
